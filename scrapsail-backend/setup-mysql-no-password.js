...
async function setupDatabase() {
  try {
    ...
    try {
      ...
      console.log('2. Root user has no password (or set DB_PASSWORD in .env)');
      console.log('3. MySQL is accessible on localhost');
      console.log('\nTo fix this, you can:');
      console.log('- Set a password for root user: ALTER USER "root"@"localhost" IDENTIFIED BY "yourpassword";');
      console.log('- Or update DB_PASSWORD in .env file');
      console.log('- Or create a new user with no password');
      process.exit(1);
    }
